...
class Prediction_Data_validation:
    ...
    def __init__(self,path):
        ...
        self.schema_path = 'Phishing/schema_prediction.json'
    ...
    def createDirectoryForGoodBadRawData(self):
        ...
        try:
            ...
            if not os.path.isdir(path):
                os.makedirs(path)
            path = os.path.join("Phishing/Prediction_Raw_Files_Validated/", "Bad_Raw/")
            if not os.path.isdir(path):
                os.makedirs(path)
                ...
                if (re.match(regex, filename)):
                    ...
                    if len(splitAtDot[1]) == LengthOfDateStampInFile:
                        if len(splitAtDot[2]) == LengthOfTimeStampInFile:
                            shutil.copy("Prediction_Batch_files/" + filename, "Phishing/Prediction_Raw_Files_Validated/Good_Raw")
                            self.logger.log(f,"Valid File name!! File moved to GoodRaw Folder :: %s" % filename)

                        else:
                            shutil.copy("Prediction_Batch_files/" + filename, "Phishing/Prediction_Raw_Files_Validated/Bad_Raw")
            ...
            for file in listdir('Phishing/Prediction_Raw_Files_Validated/Good_Raw/'):
                ...
                for columns in csv:
                    if (len(csv[columns]) - csv[columns].count()) == len(csv[columns]):
                        count+=1
                        shutil.move("Phishing/Prediction_Raw_Files_Validated/Good_Raw/" + file,
                                    "Phishing/Prediction_Raw_Files_Validated/Bad_Raw")
                        self.logger.log(f,"Invalid Column Length for the file!! File moved to Bad Raw Folder :: %s" % file)
                        break
                if count==0:
                    csv.rename(columns={"Unnamed: 0": "Wafer"}, inplace=True)
